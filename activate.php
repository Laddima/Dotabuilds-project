...
if($x && strlen($y)==32){
    ...
    if($stmt->num_rows == 1){
        ...
        if($active == $y){
            ...
            if($stmt->affected_rows == 1){
                ...
                $msg = '<p>Your account could not be activated.'
    ...
    $stmt->close(); unset($stmt); $dbcl->close(); unset($dbcl);
} // end of get request
        ...
        if($result[0]){ // record exist!
            if($result[1]!=NULL){ // account is not active
                ...
                if(send($id, $dbcl)){
                    ...
                    return '<p>System error occured, We apologize for any inconvenience.</p>';
                }
            }else{
                return '<p>Your account is already activated!</p>';
            }
        }else{
            return '<p>Please try again. There is no such record with this username/e-mail!</p>';
        }
    }
