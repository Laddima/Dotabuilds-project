...
if(filter_input(INPUT_POST,'register')){
    ...
    if(between($safeData['first_name'],2,20)){
        ...
        $errors['first_name']='Please enter your first name, between 2-20 characters';
    }
    
    if(between($safeData['last_name'],2,40)){
        $ln = $dbcl->real_escape_string($safeData['last_name']);
    }else{
        $errors['last_name']='Please enter your last name, between 2-40 characters';
    }
    
    if(between($safeData['username'],5,50)){
        $un = $dbcl->real_escape_string($safeData['username']);
    }else{
        $errors['username']='Please enter your username, between 5-50 characters';
    }
    ...
    if(between($safeData['password'],8,20)){
        ...
        $errors['password']='Please enter your password, between 8-20 characters';
    }
    ...
    if($fn && $ln && $un && $p && $e){
        ...
        if($un == $cun){
            $taken = TRUE;
            $errors['username']='Sorry, this username already exists on the website'
                    . ',if you forgot your password click here '
                    . '<a href="reset_password.php">Reset Password</a>';
        }
        if($e == $ce){
            $taken = TRUE;
            $errors['email'] = 'Sorry, this email already exists on the website,'
                    . 'if you forgot your password click here '
                    . '<a href="reset_password.php">Reset password</a>';
        }
        ...
        if(!$taken){
            ...
            if($stmt->affected_rows == 1){
                $body = "Thank you for registering at dotabuilds.000webhostapp.com."
                ...
                $_SESSION['msg'] = '<p>Thank you for registering! '
                        ...
                        . ' Please follow the link to verify your email and activate your account.';
                $stmt->close();
                unset($stmt);
                $dbcl->close();
                unset($dbcl);
                ...
                $general_msg = '<p>System error occured,'
                        . 'Your record couldn\'d be registered at this time, we apologize'
